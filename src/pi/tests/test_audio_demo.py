...
class AudioDemoTests(unittest.TestCase):
    def test_read_cpu_temp(self):
        ...
        self.assertEqual(value, {"cpuTempC": 45.0})

    def test_transmit_invokes_minimodem_with_default_baud(self):
        with mock.patch("subprocess.run") as run_mock:
            audio_demo.transmit("tempC=21.5")
        run_mock.assert_called_once()
        args, kwargs = run_mock.call_args
        self.assertEqual(args[0], ["minimodem", "--tx", "--quiet", "1200"])
        self.assertEqual(kwargs["input"], b"tempC=21.5\n")
