...
def _wake_screen(transport, label: str) -> None:
    ...
    can't be bypassed via adb — the test will surface that as a foreground-activity
    error from the reader scenario."""
...
def _wait_activity_resumed(device, local_port: int, *, label: str, timeout: float = 15.0) -> None:
    """Probe the reader scenario with a 1s timeout until it stops reporting
    'no foreground activity' — meaning MainActivity.Current has attached."""
    ...
    while time.monotonic() < deadline:
        try:
            r = requests.post(
                ...
                json={"aidHex": HCE_AID_HEX, "timeoutMs": 200},
                timeout=5,
            )
            if r.ok:
            ...
            pass
        time.sleep(0.5)
    raise TimeoutError(f"{label}: MainActivity never reached foreground (last={last_reason!r})")
